...
尽管每一次都需要构造完整的虚拟DOM树，但是因为虚拟DOM是内存数据，性能是极高的，而对实际DOM进行操作的仅仅是Diff部分，因而能达到提高性能的目的。这样，在保证性能的同时，开发者将不再需要关注某个数据的变化如何更新到一个或多个具体的DOM元素，而只需要关心在任意一个数据状态下，整个界面是如何Render的。数据驱动，声明式
...
[文档](https://segme-ntfault.com/a/1190000000357534)
```
...
当我们要进行二次配置的时候，需要找到node_modules文件夹里的react-scripts进行配置，但是当我们执行npm run eject就可以将配置抽出，方便开发
...
13. 请列举你所了解的react中的性能优化
    ...
    * shouldComponentUpdate来判断组件是否需要重新render
...
29. 使用ES6创建react组件的方式是（class ...  extends React.Component）,ES5创建组件的方法是(React.createClass),创建无状态组件的方式是(function(props){})
