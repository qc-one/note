...
(4)组件dom添加样式
	...
	其实我们大多数情况下还是大量的在为元素添加类名、id以使用某些样式，但是需要注意的是，class需要写成className（因为毕竟是在写类js代码，会收到js规则的限制，而class是关键字）
...
(10)React中的事件对象
react中对于事件进行了处理，解决了一些兼容性问题，react事件对象上面挂载着e.nativeEvent，这个就是原生的事件对象
...
(14)webpack
	...
	首先,全局安装webpack:cnpm install webpack -g
	将webpack看成一个项目,初始化项目:cnpm init -y,下载webpack:cnpm install webpack -D
	使用webpack运行,提示安装webpack-cli,cnpm install webpack-cli -D
	
	坑:
		执行完cnpm install webpack-cli -D,还会提示要安装webpack-cli,https://blog.csdn.net/qq_26780317/article/details/88554399
		这是因为在webpack 3中，webpack本身和它的CLI以前都是在同一个包中，但在第4版中，他们已经将两者分开来更好地管理它们。尝试全局安装webpack-cli,cnpm install webpack-cli -g
	...
	### plugins
	在webpack编译用的是loader，但是有一些loader无法完成的任务，交由插件（plugin）来完成，使用插件的时候需要在配置项中配置plugins选项，值是数组，可以放入多个插件的使用，而一般的插件都是一个构造器，我们只需在plugins数组中放入该插件的实例即可，在实例化插件的时候又可以传入options，对插件的使用进行配置
	...
	下载cnpm i html-webpack-plugin -D
	然后在webpack.dev.js中使用,
...
(15)ES6中的react
	在webpack中使用react,cnpm install react react-dom -S
	1.创建组件：
		在react中有两种组件,一种是有状态组件,另外一种是无状态组件
		使用class来创建组件
...
(16)%PUBLIC_URL%可以指向public里面的文件路径
...
(18)受控组件
	将状态与input的value值绑定在一起,当input的value值改变的时候,让state也改变
...
(20)在react脚手架中引用public中的文件直接  background-image: url(/logo192.png);

(21)css3高斯模糊
	filter: blur(10px);

(22)当我们进入到mine的时候判断是否有登陆信息，有的话去user，没有的话去login，在mine的componentWillMount里做的判断
...
(23)考虑到目前市场上使用redux的时候，有如下的两种情况
